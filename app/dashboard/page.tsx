...
export const metadata: Metadata = {
  ...
  description:
    "View and manage all your products in one place. Add, update, and delete inventory items with ease.",
  openGraph: {
...
export default async function Page() {
  ...
  const recentProcuts = await prisma.product.findMany({
    where: { userId },
    take: 5,
  ...
  const highStockProduct = allProducts.filter(
    (p: any) => Number(p.quantity) > 5
  ).length;

  const lowStockProduct = allProducts.filter(
    (p: any) => Number(p.quantity) <= 5 && Number(p.quantity) >= 1
  ).length;

  const outOfStockProduct = allProducts.filter(
    (p: any) => Number(p.quantity) === 0
  ).length;

  const stockPercentage =
    totalProducts > 0
      ? Math.round((highStockProduct / totalProducts) * 100)
      : 0;

  const lowStockPercentage =
    totalProducts > 0 ? Math.round((lowStockProduct / totalProducts) * 100) : 0;

  const outOfStockPercentage =
    totalProducts > 0
      ? Math.round((outOfStockProduct / totalProducts) * 100)
      : 0;
    ...
    <div className="ml-5 p-6 ">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        <div className="bg-white shadow-xl rouned-lg border-gray-300  p-6">
          <h1 className="text-purple-700 text-2xl p-3 font-bold mb-4">
            General info
          </h1>

          <div className="grid grid-cols-1 md:grid-cols-3  gap-6">
            <div className="flex flex-col items-center  justify-center">
              <h2 className="text-purple-500 text-3xl font-bold mb-2">
                {totalProducts}
              </h2>
        ...
        <div className="bg-white shadow-xl rouned-lg border-gray-300  p-6">
          ...
          <div className="w-full">
            <div className="space-y-3 ">
              {recentProcuts?.map((r: any, key: number) => {
                const stockLevel =
                  r.quantity === 0
                    ? 0
                    : r.quantity <= (r.lowStockAt || 5)
                    ? 1
                    : 2;
